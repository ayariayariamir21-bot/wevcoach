...
const emptyForm = { name: "", price: "", description: "", link: "" };
...
export default function AdminPlans() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setSuccess(editingId ? "Plan updated." : "Plan added.");
      setShowForm(false);
      load();
    } catch (err) {
      setError(err.message);
      ...
      {showForm && (
        ...
        >
          <label className="flex flex-col gap-1 text-sm font-medium">
            Name *
            <input required value={form.name} onChange={(e) => set("name", e.target.value)} placeholder="Plan name" className={inputClass} />
          </label>
          <label className="flex flex-col gap-1 text-sm font-medium">
            Price ($) *
            <input required type="number" min="0" step="0.01" value={form.price} onChange={(e) => set("price", e.target.value)} placeholder="49" className={inputClass} />
          </label>
          <label className="flex flex-col gap-1 text-sm font-medium sm:col-span-2">
            Description
            <textarea value={form.description} onChange={(e) => set("description", e.target.value)} placeholder="What's included..." rows={3} className={inputClass} />
          </label>
          <label className="flex flex-col gap-1 text-sm font-medium sm:col-span-2">
            Link
        ...
        <table className="w-full min-w-[640px] text-left text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={4} className="px-4 py-6 text-center text-gray-400">No plans yet.</td></tr>
            ) : (
              items.map((p) => (
